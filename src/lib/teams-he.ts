/**
 * טבלת תרגום שמות קבוצות ומדינות לעברית
 * מקור: UCL 2025/2026 + ליגות אחרות + גביע העולם
 *
 * להוסיף תרגום חדש: פשוט הוסף שורה בפורמט:
 *   'שם באנגלית': 'שם בעברית',
 */

export const TEAMS_HE: Record<string, string> = {

  // ── ליגת האלופות UCL 2025/2026 ──────────────────────────────────

  // ספרד
  'Real Madrid':       'ריאל מדריד',
  'Barcelona':         'ברצלונה',
  'Atletico Madrid':   'אתלטיקו מדריד',
  'Athletic Club':     'אתלטיק בילבאו',
  'Villarreal':        'ויארריאל',
  'Sevilla':           'סוויליה',

  // אנגליה
  'Arsenal':           'ארסנל',
  'Chelsea':           'צ׳לסי',
  'Liverpool':         'ליברפול',
  'Manchester City':   'מנצ׳סטר סיטי',
  'Manchester United': 'מנצ׳סטר יונייטד',
  'Newcastle':         'ניוקאסל',
  'Tottenham':         'טוטנהאם',
  'Aston Villa':       'אסטון וילה',

  // גרמניה
  'Bayern München':    'באיירן מינכן',
  'Borussia Dortmund': 'בורוסיה דורטמונד',
  'Bayer Leverkusen':  'באיירן לברקוזן',
  'Eintracht Frankfurt': 'איינטרכט פרנקפורט',
  'RB Leipzig':        'RB לייפציג',
  'Borussia Mönchengladbach': 'בורוסיה מ. גלדבאך',

  // איטליה
  'Inter':             'אינטר מילאן',
  'Juventus':          'יובנטוס',
  'Napoli':            'נאפולי',
  'Atalanta':          'אטלנטה',
  'AC Milan':          'AC מילאן',
  'Roma':              'רומא',
  'Lazio':             'לאציו',

  // צרפת
  'Paris Saint Germain': 'פריס סן ז׳רמן',
  'Monaco':            'מונאקו',
  'Marseille':         'מרסיי',
  'Nice':              'ניס',
  'Lyon':              'ליון',
  'Lille':             'ליל',

  // פורטוגל
  'Sporting CP':       'ספורטינג ליסבון',
  'Benfica':           'בנפיקה',
  'Porto':             'פורטו',

  // הולנד
  'Ajax':              'אייאקס',
  'PSV Eindhoven':     'PSV איינדהובן',
  'Feyenoord':         'פיינורד',

  // בלגיה
  'Club Brugge KV':    'קלוב ברוז',
  'Union St. Gilloise': 'יוניון סנט ז׳ילואז',
  'Anderlecht':        'אנדרלכט',

  // סקוטלנד
  'Celtic':            'סלטיק',
  'Rangers':           'ריינג׳רס',

  // טורקיה
  'Galatasaray':       'גלטסראיי',
  'Fenerbahçe':        'פנרבחצ׳ה',
  'Besiktas':          'בשיקטאש',

  // אוסטריה
  'Red Bull Salzburg': 'רד בול זלצבורג',
  'Sturm Graz':        'שטורם גראץ',

  // שווייץ
  'Servette FC':       'סרווט',
  'FC Basel 1893':     'בזל',
  'BSC Young Boys':    'יאנג בויז',

  // דנמרק
  'FC Copenhagen':     'קופנהגן',

  // נורווגיה
  'Bodo/Glimt':        'בודו/גלימט',
  'Brann':             'ברן',

  // קרואטיה
  'HNK Rijeka':        'ריייקה',
  'Dinamo Zagreb':     'דינמו זאגרב',

  // סרביה
  'FK Crvena Zvezda':  'כוכב אדום בלגרד',

  // צ׳כיה
  'Slavia Praha':      'סלביה פראג',
  'Plzen':             'ויקטוריה פלזן',

  // סלובקיה
  'Slovan Bratislava': 'סלובן ברטיסלאבה',

  // הונגריה
  'Ferencvarosi TC':   'פרנץ וארוש',

  // פולין
  'Lech Poznan':       'לך פוזנן',

  // בולגריה
  'Ludogorets':        'לודוגורץ',

  // רומניה
  'FCSB':              'FCSB (סטאואה)',

  // יוון
  'Olympiakos Piraeus': 'אולימפיאקוס',
  'Panathinaikos':     'פנאתינאיקוס',

  // ── ליגת העל הישראלית + ליגה לאומית ────────────────────────────

  'Maccabi Tel Aviv':          'מכבי תל אביב',
  'Maccabi Haifa':             'מכבי חיפה',
  'Hapoel Beer Sheva':         'הפועל באר שבע',
  'Maccabi Petah Tikva':       'מכבי פתח תקווה',
  'Hapoel Tel Aviv':           'הפועל תל אביב',
  'Beitar Jerusalem':          'בית"ר ירושלים',
  'Hapoel Jerusalem':          'הפועל ירושלים',
  'Hapoel Katamon':            'הפועל ירושלים',
  'Maccabi Bnei Raina':        'מכבי בני ריינה',
  'Maccabi Bnei Reineh':       'מכבי בני ריינה',
  'Bnei Sakhnin':              'בני סכנין',
  'FC Bnei Sakhnin':           'בני סכנין',
  'Hapoel Haifa':              'הפועל חיפה',
  'Ironi Kiryat Shmona':       'עירוני קריית שמונה',
  'Hapoel Acre':               'הפועל עכו',
  'Hapoel Nof HaGalil':        'הפועל נוף הגליל',
  'Hapoel Nazareth Illit':     'הפועל נוף הגליל',
  'Maccabi Netanya':           'מכבי נתניה',
  'Maccabi Umm al-Fahm':       'מכבי אום אל-פחם',
  'Hapoel Umm al-Fahm':        'הפועל אום אל-פחם',
  'Sektzia Nes Tziona':        'סקציה נס ציונה',
  'Ashdod SC':                 'מ.ס. אשדוד',
  'Ashdod':                    'מ.ס. אשדוד',
  'MS Ashdod':                 'מ.ס. אשדוד',
  'FC Ashdod':                 'מ.ס. אשדוד',
  'Ironi Tiberias':            'עירוני טבריה',
  'Hapoel Rishon LeZion':      'הפועל ראשון לציון',
  'Hapoel Petah Tikva':        'הפועל פתח תקווה',
  'Hapoel Ramat Gan':          'הפועל רמת גן',
  'Hapoel Kfar Saba':          'הפועל כפר סבא',
  'Hapoel Kfar Shalem':        'הפועל כפר שלם',
  'Hapoel Hadera':             'הפועל חדרה',
  'Hapoel Afula':              'הפועל עפולה',
  "Hapoel Ra'anana":           'הפועל רעננה',
  'Hapoel Raanana':            'הפועל רעננה',
  'Kafr Qasim':                'הפועל כפר קאסם',
  'Kiryat Yam SC':             'מ.ס. קריית ים',
  'Maccabi Herzliya':          'מכבי הרצליה',
  'Maccabi Kfar Kana':         'מכבי כפר כנא',
  'Maccabi Kabilio Jaffa':     'מכבי קביליו יפו',
  'Hapoel Ashkelon':           'הפועל אשקלון',
  'Hapoel Ramat HaSharon':     'הפועל רמת השרון',
  'Bnei Yehuda':               'בני יהודה תל אביב',
  'Bnei Yehuda Tel Aviv':      'בני יהודה תל אביב',
  'Ironi Nesher':              'עירוני נשר',
  'Ironi Nahariya':            'עירוני נהריה',
  'Ironi Modi''in':             'עירוני מודיעין',
  'Ironi Modiin':              'עירוני מודיעין',
  'Shimshon Tel Aviv':         'שמשון תל אביב',

  // אוקראינה
  'Dynamo Kyiv':       'דינמו קייב',
  'Shakhtar Donetsk':  'שחטאר דונייצק',

  // אזרביג׳ן
  'Qarabag':           'קאראבאג',

  // קזחסטן
  'Kairat Almaty':     'קאיראט אלמאטי',

  // ג׳ורג׳יה
  'Saburtalo':         'סאבורטאלו',

  // סלובניה
  'Olimpija Ljubljana': 'אולימפיה ליובליאנה',

  // פינלנד
  'KuPS':              'KuPS (קואופיו)',

  // איסלנד
  'Breidablik':        'ברידבליק',
  'Vikingur Gota':     'וויקינגר',

  // אירלנד
  'Shelbourne':        'שלבורן',

  // בוסניה
  'Zrinjski':          'זרינסקי',

  // מקדוניה
  'Shkendija':         'שקנדיה',

  // מלטה
  'Hamrun Spartans':   'האמרון ספרטנס',

  // קפריסין
  'Pafos':             'פאפוס',

  // אסטוניה
  'FC Levadia Tallinn': 'לווידיה טאלין',

  // ליטא
  'FK Zalgiris Vilnius': 'ז׳לגיריס וילנה',

  // לטביה
  'Rīgas FS':          'ריגה FS',

  // מולדובה
  'Milsami Orhei':     'מילסאמי',

  // בלארוס
  'Dinamo Minsk':      'דינמו מינסק',

  // גיברלטר
  'Lincoln Red Imps FC': 'לינקולן',

  // לוקסמבורג
  'FC Differdange 03': 'דיפרדנג׳',

  // אנדורה
  'Inter Club d\'Escaldes': 'אינטר אסקלדס',

  // גרנסי (ג׳רזי)
  'Virtus':            'ווירטוס',

  // ויילס
  'The New Saints':    'דה ניו סיינטס',

  // צפון אירלנד
  'Linfield':          'לינפילד',

  // ארמניה
  'FC Noah':           'נואה ירוון',

  // מונטנגרו
  'Buducnost Podgorica': 'בודוצ׳נוסט',

  // אלבניה
  'Egnatia Rrogozhinë': 'אגנטיה',

  // קוסובו
  'Drita':             'דריטה',

  // ── גביע העולם 2026 ─────────────────────────────────────────────

  'Argentina':         'ארגנטינה',
  'France':            'צרפת',
  'Brazil':            'ברזיל',
  'England':           'אנגליה',
  'Germany':           'גרמניה',
  'Spain':             'ספרד',
  'Portugal':          'פורטוגל',
  'Netherlands':       'הולנד',
  'Belgium':           'בלגיה',
  'Italy':             'איטליה',
  'Uruguay':           'אורוגוואי',
  'Croatia':           'קרואטיה',
  'Morocco':           'מרוקו',
  'Senegal':           'סנגל',
  'USA':               'ארה״ב',
  'Mexico':            'מקסיקו',
  'Colombia':          'קולומביה',
  'Ecuador':           'אקוודור',
  'Japan':             'יפן',
  'South Korea':       'קוריאה הדרומית',
  'Australia':         'אוסטרליה',
  'Saudi Arabia':      'ערב הסעודית',
  'Iran':              'איראן',
  'Ghana':             'גאנה',
  'Cameroon':          'קמרון',
  'Tunisia':           'תוניסיה',
  'Nigeria':           'ניגריה',
  'Egypt':             'מצרים',
  'Poland':            'פולין',
  'Switzerland':       'שווייץ',
  'Denmark':           'דנמרק',
  'Serbia':            'סרביה',
  'Czech Republic':    'צ׳כיה',
  'Austria':           'אוסטריה',
  'Turkey':            'טורקיה',
  'Türkiye':           'טורקיה',
  'Bosnia & Herzegovina': 'בוסניה הרצגובינה',
  'Bosnia':            'בוסניה הרצגובינה',
  'Ukraine':           'אוקראינה',
  'Hungary':           'הונגריה',
  'Romania':           'רומניה',
  'Sweden':            'שוודיה',
  'Norway':            'נורווגיה',
  'Finland':           'פינלנד',
  'Scotland':          'סקוטלנד',
  'Wales':             'ויילס',
  'Ireland':           'אירלנד',
  'Greece':            'יוון',
  'Albania':           'אלבניה',
  'Slovakia':          'סלובקיה',
  'Slovenia':          'סלובניה',
  'Bulgaria':          'בולגריה',
  'Israel':            'ישראל',
  'Canada':            'קנדה',
  'Jamaica':           'ג׳מייקה',
  'Costa Rica':        'קוסטה ריקה',
  'Panama':            'פנמה',
  'Paraguay':          'פרגוואי',
  'Chile':             'צ׳ילה',
  'Peru':              'פרו',
  'Bolivia':           'בוליביה',
  'Venezuela':         'ונצואלה',
  'Algeria':           'אלג׳יריה',
  'Mali':              'מאלי',
  'Ivory Coast':       'חוף השנהב',
  "Cote d'Ivoire":     'חוף השנהב',
  'South Africa':      'דרום אפריקה',
  'Congo DR':          'קונגו',
  'Qatar':             'קטאר',
  'Iraq':              'עיראק',
  'Jordan':            'ירדן',
  'Uzbekistan':        'אוזבקיסטן',
  'New Zealand':       'ניו זילנד',
  'Haiti':             'האיטי',
  'Curacao':           'קוראסאו',
  'Curaçao':           'קוראסאו',
  'Cape Verde':        'כף ורדה',
  'Cape Verde Islands': 'כף ורדה',

  // ── ליגות מקומיות נוספות ─────────────────────────────────────────

  // פרמייר ליג — שמות מלאים כפי שמגיעים מ-API-Football
  'Newcastle United':             'ניוקאסל',
  'Tottenham Hotspur':            'טוטנהאם',
  'West Ham United':              'ווסטהאם',
  'Wolverhampton Wanderers':      'וולוורהמפטון',
  'Brighton & Hove Albion':       'ברייטון',
  'Ipswich Town':                 'איפסוויץ',
  'Luton Town':                   'לוטון',
  'Burnley':                      'ברנלי',
  'Sheffield Utd':                'שפילד יונייטד',
  'Sheffield United':             'שפילד יונייטד',
  'Bournemouth':                  'בורנמות׳',
  'AFC Bournemouth':              'בורנמות׳',

  // UCL — שמות API-Football שונים מהטבלה
  'Paris Saint-Germain':          'פריס סן ז׳רמן',
  'Bayern Munich':                'באיירן מינכן',
  'Inter Milan':                  'אינטר מילאן',
  'Internazionale':               'אינטר מילאן',
  'Club Brugge':                  'קלוב ברוז',
  'Atlético Madrid':              'אתלטיקו מדריד',
  'Borussia M\'gladbach':         'בורוסיה מ. גלדבאך',
  'Crvena Zvezda':                'כוכב אדום בלגרד',
  'Red Star Belgrade':            'כוכב אדום בלגרד',
  'Shakhtar':                     'שחטאר דונייצק',
  'Viktoria Plzen':               'ויקטוריה פלזן',
  'Young Boys':                   'יאנג בויז',
  'Salzburg':                     'רד בול זלצבורג',

  // לה-ליגה
  'Alaves':                       'אלאבס',
  'Deportivo Alaves':             'אלאבס',
  'Leganes':                      'לגאנס',
  'CD Leganes':                   'לגאנס',
  'Las Palmas':                   'לאס פאלמאס',
  'UD Las Palmas':                'לאס פאלמאס',
  'Mallorca':                     'מיורקה',
  'RCD Mallorca':                 'מיורקה',
  'Rayo Vallecano':               'ראיו ואלקאנו',
  'Valladolid':                   'ואלאדוליד',

  // בונדסליגה
  'Augsburg':                     'אאוגסבורג',
  'FC Augsburg':                  'אאוגסבורג',
  'Bochum':                       'בוכום',
  'VfL Bochum':                   'בוכום',
  'Darmstadt':                    'דארמשטאט',
  'Heidenheim':                   'היידנהיים',
  'FC Heidenheim':                'היידנהיים',
  'Kiel':                         'קיל',
  'Holstein Kiel':                'קיל',
  'FC Köln':                      'קלן',
  'Koln':                         'קלן',
  'St. Pauli':                    'סנקט פאולי',

  // סרי A
  'Cagliari':                     'קאליארי',
  'Como':                         'קומו',
  'Genoa':                        'ג׳נואה',
  'Hellas Verona':                'וורונה',
  'Lecce':                        'לצ׳ה',
  'Monza':                        'מונזה',
  'Parma':                        'פארמה',
  'Venezia':                      'ונציה',
  'Real Betis':        'ריאל בטיס',
  'Real Sociedad':     'ריאל סוסייסדד',
  'Girona':            'ג׳ירונה',
  'Osasuna':           'אוסאסונה',
  'Getafe':            'חטאפה',
  'Valencia':          'ולנסיה',
  'Celta Vigo':        'סלטה ויגו',
  'Espanyol':          'אספניול',
  'Wolfsburg':         'וולפסבורג',
  'Freiburg':          'פרייבורג',
  'Hoffenheim':        'הופנהיים',
  'Mainz':             'מיינץ',
  'Union Berlin':      'יוניון ברלין',
  'Werder Bremen':     'ורדר ברמן',
  'Stuttgart':         'שטוטגרט',
  'Lens':              'לנס',
  'Rennes':            'רן',
  'Strasbourg':        'שטרסבורג',
  'Nantes':            'נאנט',
  'Reims':             'ריים',
  'Torino':            'טורינו',
  'Fiorentina':        'פיורנטינה',
  'Bologna':           'בולוניה',
  'Udinese':           'אודינזה',
  'Sassuolo':          'ססואולו',
  'Empoli':            'אמפולי',
  'Verona':            'וורונה',
  'Vitoria SC':        'ויטוריה',
}

/**
 * מחזיר את שם הקבוצה בעברית אם קיים, אחרת מחזיר את השם המקורי
 */
export function translateTeam(name: string): string {
  return TEAMS_HE[name] ?? name
}
